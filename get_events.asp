...
strSQL = "SELECT Title, DATE_FORMAT(StartDate, '%Y-%m-%d') AS StartDate, DATE_FORMAT(EndDate, '%Y-%m-%d') AS EndDate FROM events"
Set rsEvents = conn.Execute(strSQL)

If Not rsEvents.EOF Then
    events = "[" ' Inizia l'array JSON
    Do While Not rsEvents.EOF
        events = events & "{""title"":""" & rsEvents("Title") & """, ""start"":""" & rsEvents("StartDate") & """, ""end"":""" & rsEvents("EndDate") & """}"
        rsEvents.MoveNext
        If Not rsEvents.EOF Then
